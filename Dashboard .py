...
INEGI_TOKEN = ""
INEGI_API_URL = ("https://www.inegi.org.mx/app/api/indicadores/desarrolladores/"
                 "jsonxml/INDICATOR/{indicador}/es/0700/false/BISE/2.0/{token}?type=json")
...
def fetch_inegi_indicador(nombre: str, timeout: float = 4.0):
    info = INEGI_INDICADORES[nombre]
    if not INEGI_TOKEN:
        return info["respaldo"], info["unidad"], info["fuente"], False

    url = INEGI_API_URL.format(indicador=info["id"], token=INEGI_TOKEN)
    try:
        with urllib.request.urlopen(url, timeout=timeout) as resp:
            data = json.loads(resp.read().decode("utf-8"))
        valor = data["Series"][0]["OBSERVATIONS"][-1]["OBS_VALUE"]
        return valor, info["unidad"], info["fuente"], True
    except (urllib.error.URLError, KeyError, IndexError, ValueError, TimeoutError):
        return info["respaldo"], info["unidad"], info["fuente"], False


def fetch_all_inegi_indicadores():
    resultados = {}
    for nombre in INEGI_INDICADORES:
        resultados[nombre] = fetch_inegi_indicador(nombre)
    return resultados
...
PROBLEMATICAS = [
    {
        ...
        "estados_pct": {"Morelos": 90.1, "Tabasco": 89.8, "Guanajuato": 88.5, "Zacatecas": 80.2, "México": 78.0},
    },
    {
    ...
    },
    {
        ...
        "estados_pct": {"Chiapas": 67.4, "Guerrero": 60.4, "Oaxaca": 58.4, "Puebla": 54.0, "Veracruz": 50.4},
    },
    {
        "nombre": "Desempleo",
        "categoria": "Economia",
...
def get_estados_pct(problema):
    if "estados_pct" in problema:
        return problema["estados_pct"]
    return {e: round(min(99.9, problema["indice"] * w), 1) for e, w in problema["estados"].items()}
...
class MapWindow:
    def __init__(self, master, problema):
        ...
        tk.Label(self.win, text=problema["nombre"], font=(FONT_FAM, 15, "bold"),
                 bg=BG, fg=TEXT, wraplength=780).pack(pady=(14, 2))
        tk.Label(self.win, text="Porcentaje reportado por entidad federativa "
                                 "(zonas en rojo = mayor porcentaje)",
                 font=(FONT_FAM, 10), bg=BG, fg=SUBTEXT).pack()

        fig, ax = plt.subplots(figsize=(7, 5.6))
        geo = _load_mexico_geo()
        pct = get_estados_pct(problema)
        cmap = cm.get_cmap("Reds")
        norm = matplotlib.colors.Normalize(vmin=0, vmax=100)

        for name, rings in geo.items():
            valor = pct.get(name, 0)
            if valor > 0:
        ...
        cbar.set_label("Porcentaje reportado (%)", fontsize=8)
        ...
        ranking_txt = "Entidades con mayor porcentaje reportado: " + ", ".join(
            f"{n} ({v:g}%)" for n, v in ranked[:5]
        )
        tk.Label(self.win, text=ranking_txt, font=(FONT_FAM, 9, "bold"),
                 bg=BG, fg=TEXT, wraplength=780, justify="left").pack(pady=(0, 2), padx=14)

        tk.Label(self.win, text="Fuente: " + problema["fuente"], font=(FONT_FAM, 8),
                 bg=BG, fg=SUBTEXT, wraplength=780, justify="left").pack(pady=(0, 8), padx=14)

        botones = tk.Frame(self.win, bg=BG)
        botones.pack(pady=(0, 14))

        tk.Button(botones, text="Comparativo por entidad", command=self._show_comparativo,
                  font=(FONT_FAM, 10, "bold"), bg=ACCENT, fg="white", bd=0,
                  relief="flat", cursor="hand2", padx=20, pady=8).pack(side="left", padx=6)

        tk.Button(botones, text="Cerrar", command=self.win.destroy,
                  font=(FONT_FAM, 10, "bold"), bg=BORDER, fg=TEXT, bd=0,
                  relief="flat", cursor="hand2", padx=24, pady=8).pack(side="left", padx=6)

    def _show_comparativo(self):
        EstadosCompareWindow(self.win, self.problema)


class EstadosCompareWindow:
    def __init__(self, master, problema):
        self.win = tk.Toplevel(master)
        self.win.title(f"Comparativo por entidad - {problema['nombre']}")
        self.win.geometry("620x680")
        self.win.configure(bg=BG)
        self.win.grab_set()
        self.problema = problema

        tk.Label(self.win, text=problema["nombre"], font=(FONT_FAM, 13, "bold"),
                 bg=BG, fg=TEXT, wraplength=580).pack(pady=(14, 2))
        tk.Label(self.win, text="Porcentaje reportado por entidad federativa (%)",
                 font=(FONT_FAM, 10), bg=BG, fg=SUBTEXT).pack(pady=(0, 8))

        pct = get_estados_pct(problema)
        ranked = sorted(pct.items(), key=lambda x: x[1])

        fig, ax = plt.subplots(figsize=(5.6, 4.4))
        nombres = [n for n, _ in ranked]
        valores = [v for _, v in ranked]
        ax.barh(nombres, valores, color=ACCENT)
        ax.axvline(problema["indice"], color="#F59E0B", linestyle="--", linewidth=1.2)
        ax.text(problema["indice"], -0.7, "Promedio nacional",
                fontsize=7, color="#B45309", ha="center")
        ax.set_xlim(0, 100)
        ...
        canvas = FigureCanvasTkAgg(fig, master=self.win)
        canvas.draw()
        canvas.get_tk_widget().pack(fill="both", expand=True, padx=10, pady=(0, 6))

        self.msg = tk.Label(self.win, text="", font=(FONT_FAM, 9), bg=BG, fg=SUCCESS)
        self.msg.pack()

        botones = tk.Frame(self.win, bg=BG)
        botones.pack(pady=(6, 14))

        tk.Button(botones, text="Exportar a Excel", command=self._exportar,
                  font=(FONT_FAM, 10, "bold"), bg=ACCENT, fg="white", bd=0,
                  relief="flat", cursor="hand2", padx=20, pady=8).pack(side="left", padx=6)
    ...
    def _exportar(self):
        pct = get_estados_pct(self.problema)
        ranked = sorted(pct.items(), key=lambda x: -x[1])
        ...
        ws.cell(row=1, column=1, value="Problematica").font = header_font
        ws.cell(row=1, column=1).fill = header_fill
        ws.cell(row=1, column=1).alignment = center
        ws.cell(row=1, column=2, value=self.problema["nombre"]).alignment = Alignment(horizontal="left")
        ws.merge_cells(start_row=1, start_column=2, end_row=1, end_column=3)

        ws.cell(row=2, column=1, value="Fuente").font = header_font
        ws.cell(row=2, column=1).fill = header_fill
        ws.cell(row=2, column=1).alignment = center
        ws.cell(row=2, column=2, value=self.problema["fuente"]).alignment = Alignment(horizontal="left")
        ws.merge_cells(start_row=2, start_column=2, end_row=2, end_column=3)

        headers = ["Entidad federativa", "Porcentaje (%)", "Promedio nacional (%)"]
        ...
        for row_idx, (estado, valor) in enumerate(ranked, 5):
            ws.cell(row=row_idx, column=1, value=estado).border = border
            ws.cell(row=row_idx, column=2, value=valor).border = border
            ws.cell(row=row_idx, column=2).alignment = center
            ws.cell(row=row_idx, column=3, value=self.problema["indice"]).border = border
            ws.cell(row=row_idx, column=3).alignment = center
            if row_idx % 2 == 0:
                for col_idx in range(1, 4):
                    ws.cell(row=row_idx, column=col_idx).fill = PatternFill("solid", fgColor="F3F4F6")

        ws.column_dimensions["A"].width = 26
        ws.column_dimensions["B"].width = 18
        ws.column_dimensions["C"].width = 22

        nombre_limpio = "".join(c if c.isalnum() else "_" for c in self.problema["nombre"])
        archivo = f"Comparativo_{nombre_limpio}.xlsx"
        wb.save(archivo)

        self.msg.config(text=f"Archivo exportado: {archivo}")


class GraphsWindow:
    def __init__(self, master):
        self.win = tk.Toplevel(master)
        self.win.title("Panorama general - Graficas")
        self.win.geometry("1040x780")
        self.win.configure(bg=BG)
        self.win.grab_set()

        tk.Label(self.win, text="Panorama general de las problematicas",
                 font=(FONT_FAM, 15, "bold"), bg=BG, fg=TEXT).pack(pady=(14, 2))
        tk.Label(self.win, text="Resumen visual de los datos mas relevantes de las 20 problematicas",
                 font=(FONT_FAM, 10), bg=BG, fg=SUBTEXT).pack(pady=(0, 8))
        ...
        estado_totales = {}
        for p in PROBLEMATICAS:
            for e, w in p["estados"].items():
                estado_totales[e] = estado_totales.get(e, 0) + w
        top_estados = sorted(estado_totales.items(), key=lambda x: -x[1])[:10]

        ranking = sorted(PROBLEMATICAS, key=lambda p: -p["indice"])[:10]

        cat_sum = {}
        cat_cnt = {}
        for p in PROBLEMATICAS:
            cat_sum[p["categoria"]] = cat_sum.get(p["categoria"], 0) + p["indice"]
            cat_cnt[p["categoria"]] = cat_cnt.get(p["categoria"], 0) + 1
        cats = list(cat_sum.keys())
        cat_avgs = [cat_sum[c] / cat_cnt[c] for c in cats]

        fig, axes = plt.subplots(2, 2, figsize=(10, 7.6))
        fig.subplots_adjust(hspace=0.6, wspace=0.4, top=0.93, bottom=0.1, left=0.1, right=0.97)
        ...
        ax.set_title("Problematicas por categoria", fontsize=10, fontweight="bold")

        ax = axes[0, 1]
        nombres_e = [n for n, _ in top_estados][::-1]
        valores_e = [v for _, v in top_estados][::-1]
        ax.barh(nombres_e, valores_e, color=ACCENT)
        ax.set_title("Entidades mas mencionadas\n(suma de intensidades en todas las problematicas)",
                      fontsize=9, fontweight="bold")
        ax.tick_params(axis="y", labelsize=7)
        ax.tick_params(axis="x", labelsize=8)

        ax = axes[1, 0]
        nombres_p = [p["nombre"][:30] for p in ranking][::-1]
        valores_p = [p["indice"] for p in ranking][::-1]
        ax.barh(nombres_p, valores_p, color="#F59E0B")
        ax.set_title("Top 10 - Indice de incidencia nacional reportado (%)", fontsize=9, fontweight="bold")
        ax.set_xlim(0, 100)
        ax.tick_params(axis="y", labelsize=6.5)
        ax.tick_params(axis="x", labelsize=8)

        ax = axes[1, 1]
        ax.bar(cats, cat_avgs, color="#10B981")
        ax.set_title("Indice de incidencia promedio por categoria (%)", fontsize=9, fontweight="bold")
        ax.set_ylim(0, 100)
        ...
        canvas = FigureCanvasTkAgg(fig, master=self.win)
        canvas.draw()
        canvas.get_tk_widget().pack(fill="both", expand=True, padx=10, pady=(4, 4))

        tk.Label(self.win, text="Las graficas se construyen a partir de los datos y fuentes "
                                 "registrados para cada una de las 20 problematicas (ver detalle "
                                 "en el panel principal del dashboard).",
                 font=(FONT_FAM, 8), bg=BG, fg=SUBTEXT, wraplength=980, justify="left").pack(pady=(0, 4), padx=14)
...
class IndicadoresWindow:
    def __init__(self, master):
        self.win = tk.Toplevel(master)
        self.win.title("Indicadores nacionales - INEGI")
    ...
    def _build(self):
        ...
        tk.Label(self.win, text="Indicadores nacionales (INEGI)",
                 font=(FONT_FAM, 15, "bold"), bg=BG, fg=TEXT).pack(pady=(16, 2))

        if INEGI_TOKEN:
            sub = "Datos obtenidos del Banco de Indicadores del INEGI."
        else:
            sub = ("Sin token de la API del INEGI configurado: se muestran "
                   "valores de referencia. Agrega tu token gratuito en "
                   "INEGI_TOKEN para consultar datos en vivo.")
        tk.Label(self.win, text=sub, font=(FONT_FAM, 9), bg=BG, fg=SUBTEXT,
                 wraplength=500, justify="center").pack(pady=(0, 12), padx=20)

        cards = tk.Frame(self.win, bg=BG)
        cards.pack(fill="both", expand=True, padx=20)

        self.status = tk.Label(self.win, text="", font=(FONT_FAM, 9),
                                bg=BG, fg=SUBTEXT)
        self.status.pack(pady=(4, 0))

        self._cards_frame = cards
        self._render(fetch_all_inegi_indicadores())

        tk.Button(self.win, text="Actualizar", command=self._actualizar,
                  font=(FONT_FAM, 10, "bold"), bg=ACCENT, fg="white", bd=0,
                  relief="flat", cursor="hand2", padx=20, pady=8).pack(pady=(10, 4))
        tk.Button(self.win, text="Cerrar", command=self.win.destroy,
                  font=(FONT_FAM, 10, "bold"), bg=BORDER, fg=TEXT, bd=0,
                  relief="flat", cursor="hand2", padx=20, pady=8).pack(pady=(0, 14))

    def _render(self, resultados):
        for w in self._cards_frame.winfo_children():
            w.destroy()

        en_vivo = False
        for nombre, (valor, unidad, fuente, ok) in resultados.items():
            if ok:
                en_vivo = True
            card = tk.Frame(self._cards_frame, bg=CARD, highlightbackground=BORDER,
            ...
            tk.Label(inner, text=nombre, font=(FONT_FAM, 11, "bold"),
                     bg=CARD, fg=TEXT, anchor="w").pack(fill="x")
            tk.Label(inner, text=f"{valor} {unidad}", font=(FONT_FAM, 14, "bold"),
                     bg=CARD, fg=ACCENT, anchor="w").pack(fill="x", pady=(2, 0))
            estado = "Dato en vivo (INEGI)" if ok else "Valor de referencia (sin conexion)"
            tk.Label(inner, text=f"{fuente}  •  {estado}", font=(FONT_FAM, 8),
                     bg=CARD, fg=SUBTEXT, anchor="w").pack(fill="x", pady=(2, 0))

        if en_vivo:
            self.status.config(text="Datos actualizados desde la API del INEGI.")
        else:
            self.status.config(text="Mostrando valores de referencia almacenados localmente.")

    def _actualizar(self):
        self.status.config(text="Consultando API del INEGI...")
...
class DashboardWindow:
    ...
    def _build(self):
        ...
        tk.Button(header, text="📊 Panorama general", command=self._show_graphs,
                  font=(FONT_FAM, 10, "bold"), bg=ACCENT_H, fg="white",
                  bd=0, relief="flat", cursor="hand2", padx=14, pady=6).pack(side="right", padx=(0, 10))

        tk.Button(header, text="📡 Indicadores INEGI", command=self._show_indicadores,
                  font=(FONT_FAM, 10, "bold"), bg=ACCENT_H, fg="white",
                  bd=0, relief="flat", cursor="hand2", padx=14, pady=6).pack(side="right", padx=(0, 10))
    ...
    def _show_graphs(self):
        GraphsWindow(self.root)

    def _show_indicadores(self):
        IndicadoresWindow(self.root)
